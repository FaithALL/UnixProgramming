...
//描述一个子进程类，m_pid是目标子进程的PID，m_pipefd是父进程和子进程通信用的管道
class process {
public:
    pid_t m_pid;
    int m_pipefd[2]{};
...
void processpool<T>::run_child() {
    ...
    while (!m_stop) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if ((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
                int client = 0;
                //从父子进程之间的管道读取数据，并将结果保存在变量client中。如果读取成功，则表示有新客户连接的到来
    ...
    users = NULL;
    close(pipefd);
    //应该由m_listenfd的创建者来关闭这个文件描述符(见后文)，即所谓的“对象”(比如一个文件描述符，又或者一段堆内存)由哪个函数创建，就应该由哪个函数销毁
    //close(m_listenfd)
    close(m_epollfd);
}
    ...
    while (!m_stop) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == m_listenfd) {
                //如果有新连接的到来，就采用Round Robin方式将其分配给一个子进程处理
                ...
                if (ret <= 0) continue;
                    ...
                    for (int i = 0; i < ret; ++i) {
                        switch (signals[i]) {
                            ...
                            case SIGTERM:
                            case SIGINT: {
                                //如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知子进程结束更好的方式是向父、子进程之间的通信管道发送特殊数据
