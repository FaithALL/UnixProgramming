//利用线程池来实现并发的Web服务器，http_conn.h的实现
#include "http_conn.h"
#include <sys/uio.h>
...
http_conn::HTTP_CODE http_conn::pares_content(char *text) const {
    if (m_read_idx >= (m_content_lenght + m_checked_idx)) {
        text[m_content_lenght] = '\0';
        return GET_REQUEST;
    }
    return NO_REQUEST;
}
...
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    while (((m_check_state == CHECK_STATE_CONNECT) && (line_status == LINE_OK)) ||
