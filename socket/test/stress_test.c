...
bool read_once(int sockfd, char *buffer, int len) {
    ...
    printf("read in %d bytes form socket %d with content: %s\n", bytes_read, sockfd, buffer);
    return true;
}
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("usage: %s ip port connect_count\n", basename(argv[0]));
        return 1;
    }
    ...
    while (true) {
        ...
        for (int i = 0; i < fds; ++i) {
            ...
            if (events[i].events & EPOLLIN) {
                ...
                if (!write_nbytes(sockfd, request, strlen(request))) close_conn(epoll_fd, sockfd);
                struct epoll_event event;
