...
int run_child(int idx, struct client_data *users, char *share_mem) {
    ...
    //子进程使用IO复用技术同时监听两个文件描述符：客户连接socket、与父进程通信的管道文件描述符
    ...
    while (!stop_child) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == connfd && (events[i].events & EPOLLIN)) {
                ...
                //将客户数据读到对应的读缓存中。读缓存是共享内存的一段，它开始于idx*BUFFER_SIZE处，长度为BUFFER_SIZE
                //字节。因此，各个连接的读缓存是共享的
...
int main(int argc, char *argv[]) {
    ...
    user_count = 0;
    users = malloc((USER_LIMIT + 1) * sizeof(struct client_data));
    sub_process = malloc(PROCESS_LIMIT * sizeof(int));
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == listenfd) {
                ...
                if (ret <= 0) continue;
                    ...
                    //向除负责管理第child个客户连接的子进程之外的其他子进程发送消息，通知它们有客户数据要写
